...
export default function HallTab() {
    const { order, unsetStudio, hallsInvalid, setActiveTab, hallAmount, updateHallDateTime } = useCart();
    ...
    const next = () => {
        for (const hall of order.halls) {
            ...
            if (ord.startOf('day').isSame(today.startOf('day')) || ord.startOf('day').isBefore(today.startOf('day'))) {
        ...
        <div className="p-[30px] bg-[rgba(255,255,255,0.45)] md:rounded-b-[40px]">
            {order.halls.length > 0 && (
                <div className="mb-6">
                    {order.halls.map((i) => (
                        <div key={i.id}>
                            <div className="flex gap-[14px] md:gap-[24px] py-[20px] md:px-5 md:py-[30px]">
                                <Image
                                    src={i.img}
                                    alt={i.name}
                                    width={163}
                                ...
                                <div className="flex flex-col justify-evenly w-full relative">
                                    ...
                                    <h2 className="font-['ManegeDemo'] text-lg font-light md:text-4xl">{i.name}</h2>
                            ...
                            <div className="flex flex-col md:flex-row md:gap-12 border-b-[1px] border-[rgba(92,112,78,0.25)]">
                                <div className="form-group w-full">
                                    <label htmlFor={`q1-${i.id}`}>Желаемая ДАТА и ВРЕМЯ начала</label>
                                    <input
                                        type="datetime-local"
                                        id={`q1-${i.id}`}
                                        value={i.dateTime}
                                        onChange={(e) => updateHallDateTime(i.id, e.target.value)}
                                    />
                                </div>
                                <div className="form-group w-full">
            ...
            <div className="centered-buttons">
                <Link href="/booking" className="custom__btn border-[1px] border-brown">
                    Резерв студии
                </Link>
                <button
                    className="custom__btn border-[1px] border-brown disabled:border-[0] disabled:bg-transparent"
