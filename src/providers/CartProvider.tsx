...
export default function CartProvider({ children }: { children: ReactNode }) {
    ...
    const isNightOrdered = () => {
        return order.show.some(ord => ord.title.toLowerCase().includes('ночевк'));
    };
    ...
    const showTotal = () => {
        return order.show.reduce(
            (acc, s) => acc + s.price + (s.materialPrice || 0) * Number(order.anketa.answer4),
            0
        );
    };
    ...
    const orderText = (): string[] => {
        ...
        if (!isNightOrdered()) {
            ...
            let hallsDuration = order.halls.reduce((acc, h) => acc + h.duration + h.diff, 0);
        ...
        const guests = Number(order.anketa.answer4) + Number(order.anketa.answer5);
        let adminPrice = 0;
        ...
        if (order.food.length) {
            for (const [index, f] of order.food.entries()) {
                lines.push(`${index + 1}. ${f.title}:   ${f.price} * ${f.count} = ${f.count * f.price}`);
            }
    ...
    const addToCart = (item: Food) => {
        setOrder(prev => {
            ...
            if (savedItem) {
                ...
                return { ...prev, food: [...items, { ...item, count: item.minOrder }] };
            }
        });
    };
    ...
    const setStudio = (hallId: number) => {
        ...
        if (!existed) {
            ...
            const dateTime = `${t.getFullYear()}-${(t.getMonth() + 1).toString().padStart(2, '0')}-${t
                .getDate()
                .toString()
                .padStart(2, '0')}T${t.getHours().toString().padStart(2, '0')}:${t
                    .getMinutes()
                    .toString()
                    .padStart(2, '0')}`;
        ...
        setOrder(prev => ({
            ...
            halls: prev.halls.map(h => {
                if (h.id === hall.id) {
                    ...
                    return { ...h, duration: newDuration < 90 ? 90 : newDuration };
                }
                return h;
            })
        }));
